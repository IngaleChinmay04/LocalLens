...
//     // Count shops by verification status
//     const approved = shops.filter((shop) => shop.isVerified === true).length;
//     const pending = shops.filter((shop) => shop.isVerified === null).length;
//     const rejected = shops.filter((shop) => shop.isVerified === false).length;
...
async function handleGetRequest(request, user) {
  try {
    ...
    // Find shops where this user is the owner
    const shops = await Shop.find({ ownerId: userId }).lean();
